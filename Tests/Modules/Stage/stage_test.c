...
TEST_SETUP(stage_test)
{
  ...
  stage_set_feature_handler(
    (stage_feature_handler) {
      ...
      .generate_scaled_rectangles = 
        mock_lbp_feature_generate_scaled_rectangles,
...
TEST(stage_test, calculate_scaled_features_is_ok)
{
  mock_lbp_feature_expect_generate_scaled_rectangles();
  mock_lbp_feature_expect_generate_scaled_rectangles();
  mock_lbp_feature_expect_generate_scaled_rectangles();

  stage_calculate_scaled_features(&test_stage, (float*)scales, SCALES_AMOUNT);
}
